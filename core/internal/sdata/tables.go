...
func getDBInfoOnce(
	...
	g.Go(func() error {
		...
		case "oracle11g":
			row = db.QueryRowContext(qctx, oracle11gInfo)
		case "mssql":
		...
		default:
			return fmt.Errorf("unsupported database type %q: supported types are postgres, mysql, mariadb, sqlite, oracle, oracle11g, mssql, snowflake, mongodb", dbType)
		}
...
func DiscoverColumns(ctx context.Context, db *sql.DB, dbtype string, blockList []string) ([]DBColumn, error) {
	...
	default:
		return nil, fmt.Errorf("unsupported database type %q: supported types are postgres, mysql, mariadb, sqlite, oracle, oracle11g, mssql, snowflake, mongodb", dbtype)
	}
	...
	for rows.Next() {
		...
		if dbtype == "mssql" || dbtype == "oracle" || dbtype == "oracle11g" {
			c.OrigName = c.Name
			c.OrigTable = c.Table
		...
		if dbtype == "sqlite" || dbtype == "oracle" || dbtype == "oracle11g" || dbtype == "mssql" || dbtype == "snowflake" {
	...
	if hasUserViews(ctx, db, dbtype) {
		...
		// Supported: PostgreSQL, MSSQL, Oracle, Oracle11g, MySQL 8.0+.
...
func discoverCompositeFKsCSV(ctx context.Context, db *sql.DB, dbtype, query string) ([]CompositeFKInfo, error) {
	...
	normalize := dbtype == "oracle" || dbtype == "oracle11g" || dbtype == "mssql" || dbtype == "snowflake"
...
func DiscoverFunctions(ctx context.Context, db *sql.DB, dbtype string, blockList []string) ([]DBFunction, error) {
	...
	case "oracle11g":
		sqlStmt = oracle11gFunctionsStmt
	case "mssql":
