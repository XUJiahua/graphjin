...
// ParamRegisterer is an optional interface for dialects that need to record
// parameters without writing bind placeholders into the output stream.
type ParamRegisterer interface {
	RegisterParam(p Param) string
}
...
// ScopedIdentifierQuoter is an optional interface for dialects that need
// table-aware identifier quoting to preserve original database names after
// schema normalization.
type ScopedIdentifierQuoter interface {
	QuoteSchemaIdentifier(schema string) string
	QuoteTableIdentifier(table string) string
	QuoteColumnIdentifier(table, column string) string
}
